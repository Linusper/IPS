...
class RtlsUtil():
    def __init__(self, logging_file, logging_level, websocket_port=None):
        ...
        self.acc_results_raw_queue = queue.Queue()
        self.rssi_results_queue = queue.Queue()
...
#ÄNDRAT
                if msg.command == 'RTLS_CMD_ACC_ENABLE' and msg.type == "SyncRsp": #and msg.payload.status == 'RTLS_SUCCESS':
                    if msg.payload.status == 'RTLS_SUCCESS':
                        sending_node.acc_started = True
                        print("Acc enabled")
                    elif msg.payload.status == 'RTLS_FAIL':
                        print("Debugggg")

                if msg.command == 'RTLS_CMD_ACC_RESULT_RAW' and msg.type == "AsyncReq":
                    self.acc_results_raw_queue.put({
                        "X0" : msg.payload.X0,
                        "X1" : msg.payload.X1,
                        "Y0" : msg.payload.Y0,
                        "Y1" : msg.payload.Y1,
                    ...
                    logging.log(RtlsUtilLoggingLevel.ALL, msg=json.dumps(msg.payload))
                
                if msg.command == 'RTLS_CMD_ACC_RESULT' and msg.type == "AsyncReq":
                    self.acc_results_queue.put({
                        "X" : msg.payload.X,
                        "Y" : msg.payload.Y,
                        "Z" : msg.payload.Z,
                        })
                ...
                if msg.command == 'RTLS_CMD_RSSI_RESULT' and msg.type == "AsyncReq":
                    self.rssi_results_queue.put( {"rssi" : msg.payload.rssi} )
                    logging.log(RtlsUtilLoggingLevel.ALL, msg=json.dumps(msg.payload))
#TILL HIT
...
#ÄNDRAT
    ...
    def acc_start(self, nodes=None):
        nodes_to_set = [self._master_node]
        if nodes is not None:
            if isinstance(nodes, list):
                nodes_to_set = nodes
            else:
        ...
        for node in nodes_to_set:
            node.acc_started = False
            node.rtls.acc_start()

        true_cond_func = lambda nodes: all([n.acc_started for n in nodes])
    ...
    def rssi_start(self, nodes=None):
        nodes_to_set = [self._master_node]
        if nodes is not None:
            if isinstance(nodes, list):
                nodes_to_set = nodes
            ...
            self._empty_queue(self.rssi_results_queue)

        for node in nodes_to_set:
            node.rssi_started = True

        true_cond_func = lambda nodes: all([n.rssi_started for n in nodes])
        self._rtls_wait(true_cond_func, nodes_to_set, "All node start RSSI")

    def rssi_stop(self, nodes=None):
        nodes_to_set = [self._master_node]
        if nodes is not None:
            if isinstance(nodes, list):
                nodes_to_set = nodes
